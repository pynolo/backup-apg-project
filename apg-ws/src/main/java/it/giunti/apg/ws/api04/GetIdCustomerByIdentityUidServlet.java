...
/*@WebServlet(Constants.PATTERN_API04+Constants.PATTERN_GET_ID_CUSTOMER_BY_IDENTITY_UID)*/
public class GetIdCustomerByIdentityUidServlet extends ApiServlet {
	private static final long serialVersionUID = 2988847632744847679L;
	private static final String FUNCTION_NAME = Constants.PATTERN_GET_ID_CUSTOMER_BY_IDENTITY_UID;
	private static final Logger LOG = LoggerFactory.getLogger(GetIdCustomerByIdentityUidServlet.class);

	/*example testing url:
	 http://127.0.0.1:8888/api01/get_id_customer_by_cod_abbo?access_key=1234&cod_abbo=M011883&address_province=tn
	 */

    public GetIdCustomerByIdentityUidServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//acquire cod_abbo
		String identityUid = request.getParameter(Constants.PARAM_IDENTITY_UID);
		if (identityUid == null) {
			result = BaseJsonFactory.buildBaseObject(ErrorEnum.EMPTY_PARAMETER, Constants.PARAM_IDENTITY_UID+" is empty");
		}
		//build response
		if (result == null) {
			Session ses = SessionFactory.getSession();
			try {
				Anagrafiche anag = new AnagraficheDao().findByIdentityUid(ses, identityUid);
				if (anag == null) {
					throw new BusinessException(identityUid+" has no match");
				}
				//identityUid exists
