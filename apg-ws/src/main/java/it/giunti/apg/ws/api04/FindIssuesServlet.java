...
/*@WebServlet(Constants.PATTERN_API04+Constants.PATTERN_FIND_ISSUES)*/
public class FindIssuesServlet extends ApiServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (result == null) {
			...
			try {
				...
				//Elenco fascicoli
				List<MaterialiProgrammazione> fList = new MaterialiProgrammazioneDao().findByPeriodico(ses,
	...
	private JsonObjectBuilder schemaBuilder(List<MaterialiProgrammazione> fList) throws BusinessException {
		JsonBuilderFactory factory = Json.createBuilderFactory(null);
		JsonArrayBuilder arrayBuilder = factory.createArrayBuilder();
		for (MaterialiProgrammazione fas:fList) {
			JsonObjectBuilder ob = factory.createObjectBuilder();
			add(ob, "cm_issue", fas.getMateriale().getCodiceMeccanografico());
			add(ob, "nominal_issue_date", Constants.FORMAT_API_DATE.format(fas.getDataNominale()));
//			if (fas.getDataFine() != null)
//				add(ob, "nominal_issue_end", null);//TODO remove
//			add(ob, "worth", 1);//TODO remove
			add(ob, "description_number", fas.getMateriale().getTitolo());
			add(ob, "description_period", fas.getMateriale().getSottotitolo());
